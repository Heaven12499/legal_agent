...
| Agent 金标法条命中 | 29 / 30（96.7%） | 从条款审查提示到最终答案，是否引用至少一条金标法条 |
| 无效 / 无本轮依据引用 | 0 / 0 | 引用校验未发现条号不存在或脱离本轮证据的引用 |

> **不要把 100% 解读为端到端风险识别准确率。** 该检索查询已由人工按法律语义改写，测的是“法条召回”；13 条人工负样本暂不自动判分，用于人工观察误报。当前 Agent 的 29/30 是本次单次运行结果；部分重点核查点来自同一份合同的不同子条款，后续仍需要按合同分组扩样、多轮运行、双人律师标注和更完整的合同上下文复核。
