# -*- coding: utf-8 -*-
"""
检索验收脚本：固定一组测试用例（条文 + 官方案例，含已知 gold），对比
向量 / BM25 / 混合三路，判定混合 top-5 是否命中 gold，输出 PASS/FAIL 汇总。

用例分两类（gold 为检索结果的展示名）：条文用例形如"劳动合同法第四十七条"，
案例用例形如"指导案例183号"（案例与条文混同一索引，兼作加案例不挤条文的回归）。
"被裁员有没有赔偿"是已知局限、不设 gold：41 条（经济性裁员）不含"裁员/赔偿"两词，
检索层无法命中，真正的解法在 M3 的 LLM 查询改写。
"""
import sys
