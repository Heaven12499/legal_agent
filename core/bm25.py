# -*- coding: utf-8 -*-
"""
BM25 词法检索器：靠词面精确匹配找法条，和向量检索互补。

为什么还要它（M2 的核心动机）：
    向量检索强在"换说法也能懂"（同义改写、意译），但短口语化查询时
    分数会挤在一起、噪声混入。而法律问答里大量精确术语——"仲裁时效"、
    "经济补偿"、"劳务派遣"——这些词在查询和条文里**一字不差**，正好是
    BM25 词面匹配的主场，不依赖语义联想。

为什么用 jieba 分词：
    BM25 是词级别的。中文不分词，"经济补偿"就散成"经""济""补""偿"，
    既匹配不上"补偿"，也会被不相干单字带偏。jieba 切成词后，查询和
    条文在"词"上对齐。

用法：
    from core.bm25 import get_bm25
    r = get_bm25()
    for hit in r.search("仲裁时效是多久"):
        print(hit["法律"], hit["条号"])
"""
import json
